%script per la creazione della matrice Nx6 per l'oggetto Denkmit
clear all
close all
...
for i=1:height(results)
    ...
    if(~isempty(labels))
        ...
        if(iou>0.3 && strcmp(label,testTbl.Properties.VariableNames(index)))
            ...
            if(contains(testTbl.Properties.VariableNames(index),"Balea"))
                truelabels=[truelabels "Balea"];
            elseif(contains(testTbl.Properties.VariableNames(index),"Beckmann"))
...
%conseguenza.
order=["Denkmit_fronte" "Denkmit_fronte_steso","Denkmit_fronte_steso_capovolto","Denkmit_retro","Denkmit_retro_steso","Denkmit_retro_steso_capovolto","Balea","Beckmann","Finish"];
figure
